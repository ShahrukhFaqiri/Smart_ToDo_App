...
module.exports = (db) => {
  ...
  router.post("/edit", (req, res) => {
    db.query(`SELECT * FROM users WHERE name = $1;`, [req.body.submit])
      .then((data) => {
        if (data.rows.length === 0) {
          db.query(`UPDATE users SET name = $1 WHERE id = $2 RETURNING *;`, [
            req.body.submit,
            req.session.userId,
          ]).then((data) => {
  ...
  router.post("/register", (req, res) => {
    ...
    db.query(`SELECT * FROM users WHERE name = $1`, [username])
      ...
      .then((ifNewUser) => {
        if (ifNewUser) {
          db.query(`INSERT INTO users (name) VALUES ($1) RETURNING *;`, [
            username,
          ])
            .then((data) => {
      ...
      .then((data) => {
        if (data.rows.length !== 0) {
          ...
          res
            .status(403)
            .send(
              `Invalid user information, register here: <a href="/users/register">Register!</a>`
            );
        }
      })
      .catch((err) => {
